...
V0.4.2：
- 工具列文字按鈕改為緊湊寬度，第一列最小需求寬度由約 1755 px 降至約 986 px
- 手繪、清除、顯示切換、選字、方框、圖說與書籤移至第二列延伸工具
- 第二列由固定箭頭切換，展開後操作工具不會自動隱藏，再按一次箭頭才收合

V0.4.1：
- 新增純搜尋按鈕；搜尋、螢光筆與刪除線共用同一個關鍵字欄，連續搜尋可逐筆跳至命中位置
- 新增隱藏 / 顯示標記預覽，不修改 PDF 內容，並完整隱藏 Pdect 圖說向量內容
- 快速窗選清除改為帶下拉選單的工具按鈕，可一鍵清除整份文件標記並以 Ctrl+Z 回復

V0.4.0：
- 正式進版 V0.4.0，延續 V0.3.8 的 PDF 文字層選取與系統剪貼簿複製能力
- 同步整理版本化來源、歷史區與交付文件

V0.3.8：
- 改善選擇文字工具：優先讀取 PDF 文字層並寫入系統剪貼簿，可直接貼到文書編輯軟體
- 選字複製會在必要時用逐詞行序重建文字，減少貼上時多餘空白行
...
try:
    ...
    from PySide6.QtGui import QAction, QColor, QImage, QPixmap, QPainter, QPen, QKeySequence, QIcon, QClipboard
...
class PdfPageLabel(QLabel):
    ...
    def __init__(self, page_index: int, parent=None):
        ...
        self._search_result_pdf_rect = None
        self._search_result_rect: Optional[QRectF] = None
    ...
    def set_display_scale(self, scale: float):
        ...
        if self._selected_rect is not None:
            ...
            pass
        if self._search_result_pdf_rect is not None:
            self.set_search_result_pdf_rect(self._search_result_pdf_rect)
    ...
    def set_search_result_pdf_rect(self, rect: Optional[Tuple[float, float, float, float]]):
        if rect is None:
            self._search_result_pdf_rect = None
            self._search_result_rect = None
        else:
            x0, y0, x1, y1 = rect
            self._search_result_pdf_rect = (float(x0), float(y0), float(x1), float(y1))
            s = self._display_scale
            self._search_result_rect = QRectF(x0 * s, y0 * s, (x1 - x0) * s, (y1 - y0) * s)
        self.update()
    ...
    def paintEvent(self, event):
        ...
        painter = None
        if self._search_result_rect is not None:
            painter = QPainter(self)
            painter.setPen(QPen(QColor(25, 105, 210), 2, Qt.SolidLine))
            painter.setBrush(QColor(80, 155, 255, 65))
            painter.drawRect(self._search_result_rect)
        if self._selected_rect is not None:
            if painter is None:
                painter = QPainter(self)
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self._hidden_marks_doc: Optional[fitz.Document] = None
        self._hidden_preview_needs_clone: Optional[bool] = None
        self.pdf_path: Optional[Path] = None
        self.annotations_visible = True
        self._search_matches: list[tuple[int, Tuple[float, float, float, float]]] = []
        self._search_match_index = -1
        self._search_signature: Optional[tuple[str, int]] = None
        self.zoom = 1.25
    ...
    def _build_ui(self):
        ...
        secondary_toolbar = QToolBar("延伸工具列", self)
        secondary_toolbar.setMovable(False)
        secondary_toolbar.setFloatable(False)
        secondary_toolbar.setToolButtonStyle(Qt.ToolButtonIconOnly)
        ...
        self.search_edit.setToolTip("共用搜尋文字欄：搜尋、螢光筆與刪除線使用同一組文字")
        self.search_edit.returnPressed.connect(self.search_keyword)
        self.search_edit.textChanged.connect(self._reset_search_results)
        toolbar.addWidget(self.search_edit)

        self.search_btn = QPushButton("搜尋")
        self._set_compact_text_button(self.search_btn, 46)
        self.search_btn.setToolTip("搜尋下一個關鍵字命中位置；可重複按鈕逐筆瀏覽")
        ...
        self._set_compact_text_button(self.highlight_btn, 58)
        ...
        secondary_toolbar.addWidget(self.manual_highlight_btn)

        self.strike_btn = QPushButton("刪除線")
        self._set_compact_text_button(self.strike_btn, 54)
        ...
        secondary_toolbar.addWidget(self.manual_strike_btn)
        ...
        secondary_toolbar.addWidget(self.manual_diag_strike_btn)

        self.clear_marks_btn = QToolButton()
        self.clear_marks_btn.setIcon(self._make_manual_tool_icon("clear_marks"))
        self.clear_marks_btn.setFixedSize(48, 28)
        self.clear_marks_btn.setCheckable(True)
        self.clear_marks_btn.setPopupMode(QToolButton.MenuButtonPopup)
        self.clear_marks_btn.setToolTip("快速窗選清除；按右側箭頭可一鍵清除整份文件標記")
        self.clear_marks_btn.clicked.connect(
            lambda checked: self.set_manual_draw_mode("clear_marks" if checked else None)
        )
        clear_menu = QMenu(self.clear_marks_btn)
        self.clear_all_marks_action = clear_menu.addAction("清除全部標記")
        self.clear_all_marks_action.setToolTip("清除整份 PDF 的標記；整次操作可用 Ctrl+Z 回復")
        self.clear_all_marks_action.triggered.connect(self.clear_all_marks)
        self.clear_marks_btn.setMenu(clear_menu)
        secondary_toolbar.addWidget(self.clear_marks_btn)

        self.toggle_marks_btn = QPushButton("隱藏標記")
        self._set_compact_text_button(self.toggle_marks_btn, 66)
        self.toggle_marks_btn.setToolTip("只切換預覽中的標記顯示，不修改 PDF 內容")
        self.toggle_marks_btn.clicked.connect(self.toggle_annotations_visibility)
        secondary_toolbar.addWidget(self.toggle_marks_btn)

        self.select_text_btn = QPushButton("選字")
        self._set_compact_text_button(self.select_text_btn, 44)
        self.select_text_btn.setCheckable(True)
        self.select_text_btn.setToolTip("選擇文字：拖曳框選 PDF 文字，放開後自動複製到系統剪貼簿，可貼到文書編輯軟體")
        self.select_text_btn.clicked.connect(lambda checked: self.set_manual_draw_mode("select_text" if checked else None))
        secondary_toolbar.addWidget(self.select_text_btn)

        self.square_btn = QPushButton("方框")
        self._set_compact_text_button(self.square_btn, 44)
        ...
        secondary_toolbar.addWidget(self.square_btn)

        self.callout_btn = QPushButton("圖說")
        self._set_compact_text_button(self.callout_btn, 44)
        ...
        secondary_toolbar.addWidget(self.callout_btn)
        ...
        secondary_toolbar.addWidget(self.bookmark_add_btn)
        ...
        secondary_toolbar.addWidget(self.bookmark_delete_btn)
        ...
        self.bookmark_combo.setFixedWidth(140)
        ...
        secondary_toolbar.addWidget(self.bookmark_combo)

        self.more_tools_btn = QToolButton()
        self.more_tools_btn.setCheckable(True)
        self.more_tools_btn.setFixedSize(30, 28)
        self.more_tools_btn.setIcon(self.style().standardIcon(QStyle.StandardPixmap.SP_ArrowDown))
        self.more_tools_btn.setToolTip("展開第二列工具；再次按下才收合")
        self.more_tools_btn.toggled.connect(self._set_secondary_toolbar_visible)
        toolbar.addWidget(self.more_tools_btn)

        self.addToolBarBreak(Qt.TopToolBarArea)
        self.addToolBar(Qt.TopToolBarArea, secondary_toolbar)
        secondary_toolbar.hide()

        # 第一列保持精簡；第二列由固定按鈕持續展開，直到使用者再次收合。
    ...
    @staticmethod
    def _set_compact_text_button(button: QPushButton, width: int):
        button.setFixedSize(int(width), 28)
        button.setStyleSheet("QPushButton { padding: 1px 4px; margin: 0px; }")

    def _set_secondary_toolbar_visible(self, visible: bool):
        if not hasattr(self, "secondary_toolbar"):
            return
        self.secondary_toolbar.setVisible(bool(visible))
        icon = QStyle.StandardPixmap.SP_ArrowUp if visible else QStyle.StandardPixmap.SP_ArrowDown
        self.more_tools_btn.setIcon(self.style().standardIcon(icon))
        self.more_tools_btn.setToolTip(
            "收合第二列工具" if visible else "展開第二列工具；再次按下才收合"
        )

    def _lock_single_toolbar_width(self):
        """Keep the compact primary row visible without forcing an ultra-wide window."""
        if not hasattr(self, "primary_toolbar"):
            return
        required_width = self.primary_toolbar.sizeHint().width() + 20
        self.setMinimumWidth(max(900, min(1250, required_width)))
    ...
    def set_manual_draw_mode(self, mode: Optional[str]):
        ...
        if mode == "highlight":
            ...
            self._set_status("選擇文字：在 PDF 頁面拖曳框選文字，放開後自動複製到系統剪貼簿，可貼到文書編輯軟體")
        elif mode == "square":
    ...
    def _update_controls(self):
        ...
        if hasattr(self, "clear_all_marks_action"):
            self.clear_all_marks_action.setEnabled(has_doc)
        if hasattr(self, "toggle_marks_btn"):
            self.toggle_marks_btn.setEnabled(has_doc)
            self.toggle_marks_btn.setText("隱藏標記" if self.annotations_visible else "顯示標記")
        if hasattr(self, "select_text_btn"):
    ...
    def open_pdf_path(self, path: Path):
        ...
        try:
            self._invalidate_hidden_marks_doc()
    ...
    def _invalidate_hidden_marks_doc(self):
        hidden_doc = getattr(self, "_hidden_marks_doc", None)
        self._hidden_marks_doc = None
        self._hidden_preview_needs_clone = None
    ...
    def _hidden_preview_document(self):
        """Build a display-only copy with Pdect callout content streams detached."""
        if self.doc is None:
            return None
        if self._hidden_marks_doc is not None:
            return self._hidden_marks_doc

        if self._hidden_preview_needs_clone is None:
            self._hidden_preview_needs_clone = False
            for page in self.doc:
                annot = page.first_annot
                while annot:
                    if self._parse_callout_meta(annot) is not None:
                        self._hidden_preview_needs_clone = True
                        break
                    annot = annot.next
                if self._hidden_preview_needs_clone:
                    break
        if not self._hidden_preview_needs_clone:
            return self.doc

        data = self.doc.tobytes(garbage=0, deflate=False)
        hidden_doc = fitz.open(stream=data, filetype="pdf")
        marker = b"% PdectCallout-"
        for page in hidden_doc:
            contents = [int(xref) for xref in page.get_contents()]
            kept = []
            for xref in contents:
                try:
                    is_callout = hidden_doc.xref_stream(xref).startswith(marker)
                except Exception:
                    is_callout = False
                if not is_callout:
                    kept.append(xref)
            if len(kept) != len(contents):
                ...
                hidden_doc.xref_set_key(page.xref, "Contents", value)
        self._hidden_marks_doc = hidden_doc
        return hidden_doc
    ...
    def toggle_annotations_visibility(self):
        ...
        self.schedule_visible_render(0)
        self._update_controls()
        state = "顯示" if self.annotations_visible else "隱藏"
        self._set_status(f"已{state}標記｜只影響畫面預覽，不修改 PDF 內容")
    ...
    def _render_page_high_quality(self, page_index: int):
        ...
        render_doc = self.doc if self.annotations_visible else self._hidden_preview_document()
        ...
        pix = page.get_pixmap(matrix=matrix, alpha=False, annots=self.annotations_visible)
    ...
    def _reset_search_results(self, *_args):
        self._search_matches = []
        self._search_match_index = -1
        self._search_signature = None
        for label in getattr(self, "page_labels", []):
            label.set_search_result_pdf_rect(None)

    def _show_search_match(self, page_index: int, rect_values, position: int, total: int, keyword: str):
        for index, label in enumerate(self.page_labels):
            label.set_search_result_pdf_rect(rect_values if index == page_index else None)
        if len(self.page_top_positions) != len(self.page_labels):
            self._rebuild_page_positions()
        rect = fitz.Rect(*rect_values)
        target = self.page_top_positions[page_index] + int(rect.y0 * self.zoom)
        target -= max(0, self.scroll.viewport().height() // 3)
        vbar = self.scroll.verticalScrollBar()
        ...
        self._schedule_view_state_save()
        self._set_status(
            f"搜尋結果：{position} / {total}｜第 {page_index + 1} 頁｜「{keyword}」｜再按搜尋可跳到下一筆"
        )
    ...
    def search_keyword(self):
        ...
        if keyword is None:
            return
        signature = (keyword, int(self.start_page_spin.value()))
        if signature != self._search_signature or not self._search_matches:
            self._reset_search_results()
            self._search_signature = signature
            QApplication.setOverrideCursor(Qt.WaitCursor)
            try:
                start_page_index = max(0, min(len(self.doc) - 1, signature[1] - 1))
                for page_index in range(start_page_index, len(self.doc)):
                    for quad in self.doc[page_index].search_for(keyword, quads=True):
                ...
                self._reset_search_results()
                self._set_status(f"搜尋失敗：{e}")
                QMessageBox.critical(self, "搜尋失敗", f"搜尋 PDF 關鍵字時發生錯誤：\n{e}")
                return
            finally:
                QApplication.restoreOverrideCursor()

            if not self._search_matches:
                self._set_status(f"從第 {signature[1]} 頁起找不到關鍵字：「{keyword}」")
                return

        self._search_match_index = (self._search_match_index + 1) % len(self._search_matches)
        page_index, rect_values = self._search_matches[self._search_match_index]
    ...
    def _extract_text_in_rect(self, page, rect) -> str:
        # Prefer PyMuPDF's text-layer extraction because it preserves line
        # breaks and spaces better for pasting into document editors.
        try:
            ...
            text = ""
        text = self._clean_copied_text(text)
        ...
        try:
            ...
            words = []
        word_text = self._clean_copied_text(self._join_selected_words(words))
        if word_text and ("\n\n" in text or not text):
            return word_text
        return text or word_text

    @staticmethod
    def _copy_text_to_system_clipboard(text: str):
    ...
    def select_text_in_rect(self, page_index: int, x0: float, y0: float, x1: float, y1: float):
        ...
        try:
            page = self.doc[page_index]
            text = self._extract_text_in_rect(page, rect)
            if not text:
                self._set_status(f"選擇文字：第 {page_index + 1} 頁框選範圍內沒有可複製文字")
                return
            self._copy_text_to_system_clipboard(text)
            preview = text.replace("\n", " ")
            if len(preview) > 42:
                preview = preview[:42] + "..."
            self._set_status(f"已複製文字到系統剪貼簿：第 {page_index + 1} 頁｜可貼到文書編輯軟體｜{preview}")
        except Exception as e:
    ...
    def clear_marks_in_rect(self, page_index: int, x0: float, y0: float, x1: float, y1: float):
        ...
        while annot:
            try:
                ...
                elif self._annotation_counts_as_existing_mark(annot):
    ...
    def clear_all_marks(self):
        """Clear every Pdect-style mark in the document as one undoable action."""
        if self.doc is None:
            return

        page_targets: list[tuple[int, list[int], set[str]]] = []
        logical_count = 0
        for page_index in range(len(self.doc)):
            page = self.doc[page_index]
            plain_xrefs: list[int] = []
            ...
            while annot:
                ...
                annot = annot.next
            plain_xrefs = list(dict.fromkeys(plain_xrefs))
            if plain_xrefs or callout_groups:
                page_targets.append((page_index, plain_xrefs, callout_groups))
                logical_count += len(plain_xrefs) + len(callout_groups)

        if logical_count == 0:
            self._set_status("整份 PDF 沒有可清除的標記")
            return

        QApplication.setOverrideCursor(Qt.WaitCursor)
        try:
            self._capture_undo_state(f"清除全部 {logical_count} 個標記")
            deleted_total = 0
            touched_pages = set()
            for page_index, plain_xrefs, callout_groups in page_targets:
                page = self.doc[page_index]
                for group in sorted(callout_groups):
                    parts = self._callout_group_parts(page, group)
                    if parts:
            ...
            self._refresh_annotation_pages(touched_pages)
            self._set_status(
                f"已清除整份 PDF 共 {deleted_total} 個標記｜整次操作可按 Ctrl+Z 回復"
            )
        except Exception as e:
            self._set_status(f"清除全部標記失敗：{e}")
            QMessageBox.critical(self, "清除失敗", f"無法清除全部標記：\n{e}")
        finally:
            QApplication.restoreOverrideCursor()
            self._update_controls()
    ...
    def select_annotation_at(self, page_index: int, pdf_x: float, pdf_y: float):
        if self.doc is None or not self.annotations_visible or not (0 <= page_index < len(self.doc)): return
