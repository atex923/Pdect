# -*- coding: utf-8 -*-
"""
Pdect V0.4.3 recovery / quick-save extension.
...
V0.4.3:
- Align extension version with Pdect V0.4.3.

V0.4.2:
- Align extension version with Pdect V0.4.2.

V0.4.1:
- Align extension version with Pdect V0.4.1.

V0.4.0:
- Align extension version with Pdect V0.4.0.

V0.3.8:
- Align extension version with Pdect V0.3.8.

V0.3.7:
- Align extension version with Pdect V0.3.7.

V0.3.6:
- Align extension version with Pdect V0.3.6.
- Prune only unusable incomplete recovery folders after a conservative grace period.
"""
...
INCOMPLETE_RECORD_GRACE_SECONDS = 24 * 60 * 60
...
def _record_folder_is_older_than(folder: Path, seconds: int) -> bool:
    try:
        newest = max((p.stat().st_mtime for p in folder.iterdir()), default=folder.stat().st_mtime)
    ...
    def scan_recovery_records(self) -> None:
        ...
        for folder in sorted(folders):
            ...
            if not meta_path.exists() or not snapshot.exists():
                if _record_folder_is_older_than(folder, INCOMPLETE_RECORD_GRACE_SECONDS):
