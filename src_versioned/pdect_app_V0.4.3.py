...
V0.4.3：
- 取消第二列延伸工具，所有工具恢復固定在同一排
- 螢光筆恢復為文字按鈕後接手繪螢光筆；刪除線恢復為文字按鈕後接水平與斜線手繪工具
- 保留 V0.4.2 的緊湊文字按鈕與縮短欄位，單列工具完整顯示所需寬度約 1511 px
...
class MainWindow(QMainWindow):
    ...
    def _build_ui(self):
        ...
        toolbar.addWidget(self.toggle_marks_btn)
        ...
        # 所有工具固定在同一列；依完整內容設定最小寬度，避免 Qt 產生第二列或 overflow。
    ...
    def _lock_single_toolbar_width(self):
        """Keep every compact toolbar control visible on one row."""
