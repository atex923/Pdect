# Pdect / P找碴 V0.3.6 — Codex 交接

## 1. 執行方式
將以下三個檔案放在同一資料夾：

- `Pdect_V0.3.6.pyw`：防閃退啟動器，正式建議入口。
...
- 應用程式：V0.3.6
- 啟動器：V0.3.6
- Recovery / Quick Save extension：V0.3.6
...
- [ ] `python -m py_compile Pdect_V0.3.6.py pdect_app.py pdect_recovery_ext.py` 無 SyntaxError。
...
## 8. 版本規則
目前專案已到 `V0.3.6`。一般 bugfix / 小功能請增加第三碼，例如 `V0.3.7`；大型功能或架構調整再增加第二碼。
