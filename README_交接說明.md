# Pdect / P找碴 V0.4.3 — Codex 交接

## 1. 執行方式
將以下三個檔案放在同一資料夾：

- `Pdect_V0.4.3.pyw`：防閃退啟動器，正式建議入口。
...
- 應用程式：V0.4.3
- 啟動器：V0.4.3
- Recovery / Quick Save extension：V0.4.3

## 3. 主要功能
- PDF 拖曳開啟、整份連續垂直預覽。
- Ctrl + 滾輪縮放；一般滾輪捲動；左鍵拖曳平移。
- 搜尋、螢光筆與刪除線共用同一關鍵字欄；純搜尋可逐筆跳至命中位置，並可指定起始頁。
- 所有工具固定在緊湊單列；螢光筆文字按鈕後接手繪螢光筆，刪除線文字按鈕後接水平與斜線手繪工具。
...
- 略過已有標記區域。
- 隱藏／顯示標記只影響預覽，不修改 PDF 內容；含圖說時會建立只供顯示使用的副本。
- 快速窗選清除：主按鈕拖出矩形後批次刪除相交標記；右側下拉選單可清除整份 PDF 標記；兩者都只建立一筆 Undo。
...
## 4. 圖說字型的重要限制
V0.4.3 圖說文字延續不依賴一般 FreeText fallback，而是使用 Windows 標楷體實際字型檔：
...
## 6. 近期最重要改版
### V0.4.3
- 所有工具恢復固定在同一排，取消第二列與展開箭頭。
- 螢光筆恢復為文字按鈕後接手繪螢光筆；刪除線恢復為文字按鈕後接水平與斜線手繪工具。
- 保留緊湊按鈕與縮短欄位，單列完整寬度約 1511 px。
...
- 快速窗選清除改成帶右側下拉選單的工具按鈕，新增整份 PDF 一鍵清除標記並納入 Undo / Redo。
...
- [ ] `python -m py_compile Pdect_V0.4.3.py Pdect_V0.4.3.pyw pdect_app.py pdect_recovery_ext.py` 無 SyntaxError。
- [ ] 沒有 `pdect_recovery_ext.py` 時仍可啟動主程式。
- [ ] 缺少 `pdect_app.py` 時 launcher 顯示訊息而非閃退。
- [ ] 開啟一般 PDF、拖曳 PDF 均正常。
- [ ] Ctrl+滾輪縮放與普通滾輪捲動正常。
- [ ] 純搜尋可逐筆定位；同一文字欄可供螢光筆與刪除線使用。
- [ ] 工具列只有一排且所有工具完整；螢光筆與刪除線工具順序符合 V0.4.1 排列。
- [ ] 3 色色塊及 `▼` 色盤正常。
- [ ] 手繪三種線正常。
...
- [ ] 快速窗選清除只刪框內相交標記，框外保留。
- [ ] 隱藏／顯示標記不修改 PDF；全部清除可由 Ctrl+Z / Ctrl+Y 回復與重做。
...
## 8. 版本規則
目前專案已到 `V0.4.3`。一般 bugfix / 小功能請增加第三碼，例如 `V0.4.4`；大型功能或架構調整再增加第二碼。
